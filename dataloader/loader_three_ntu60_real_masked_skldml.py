...
class Feeder(Dataset):
    def __init__(self, data_path=None, label_path=None, p_interval=1, mode='train', random_choose=False, random_shift=False,
        ...
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/data_info_03_005_60.pkl', 'rb')
        self.data_info = pkl.load(f)
        f.close()    
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/SNR_info_03_005_60.pkl', 'rb')
        self.snr_info = pkl.load(f)
        f.close()                
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/key_info_03_005_60.pkl', 'rb')
        self.key_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/num_info_03_005_60.pkl', 'rb')
        self.num_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/ntu60train.pkl', 'rb')
        train_info = pkl.load(f)
        f.close()                  
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/ntu60test.pkl', 'rb')
        test_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/ntu60val.pkl', 'rb')
        val_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/ntu_60/length_03_005_60.pkl', 'rb')
        ...
        if mode == 'train':
            ...
            f.close()
        if self.mode == 'test':
            snr_list = [self.snr_info[i] for i in self.index_list] 
            import csv
            f = open('/home/haicore-project-kit-iar-cvhci/fy2374/tmm/sl-dml/snr.csv', 'w+')
            writer = csv.writer(f)
            #for item in self.snr_info:
    ...
    def __getitem__(self, ind):
        ...
        self.bone=True
        self.vel=True
        person = self.num_info[ind]
        data_numpy = self.data_info[index]
        image_path = ''+self.key_info[index]
        ...
        if self.mode == 'train':
            image_path = '/home/haicore-project-kit-iar-cvhci/fy2374/tmm/sl-dml/data/data_s47/ntu/ntu_swap_axes_testswapaxes/one_shot/train/' + 'A' + image_path.split('.')[0].split('A')[-1] + '/'+image_path.replace('npy', 'png')
        elif self.mode == 'val':
            image_path = '/home/haicore-project-kit-iar-cvhci/fy2374/tmm/sl-dml/data/data_s47/ntu/ntu_swap_axes_testswapaxes/one_shot/samples/'  + 'A' + image_path.split('.')[0].split('A')[-1] + '/'+image_path.replace('npy', 'png')
        else:
            image_path = '/home/haicore-project-kit-iar-cvhci/fy2374/tmm/sl-dml/data/data_s47/ntu/ntu_swap_axes_testswapaxes/one_shot/test/'  + 'A' + image_path.split('.')[0].split('A')[-1] + '/'+image_path.replace('npy', 'png')
        image = np.array(self.pil_loader(image_path))
        #N,T,C = image.shape
        '''
        ...
        if self.vel:
            ...
            #vel_data_numpy_masked = self.preprocess_norm(vel_data_numpy_masked, length)
        ...
        #bone_data_numpy = torch.Tensor(np.resize(bone_data_numpy,(256,256,3))).permute(2,0,1)#self.transforms(bone_data_numpy)
        #vel_data_numpy = torch.Tensor(np.resize(vel_data_numpy,(256,256,3))).permute(2,0,1) #self.transforms(vel_data_numpy)    
        return  data_numpy, torch.Tensor([label]).long() #torch.stack([data_numpy, bone_data_numpy, vel_data_numpy], dim=1).permute(0,1,2,3), torch.Tensor([label]).long()
