...
class Feeder(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.mode == 'train':
            image_path = '/hkfs/work/workspace/scratch/fy2374-blah/toyota_one_shot/toyota_one_shot/ntu/ntu_no_border/one_shot/train/' +image_path.split('/')[-1].split('_')[0] + '/' + (image_path.split('/')[-1]).replace('npy', 'png')
        elif self.mode == 'val':
            image_path = '/hkfs/work/workspace/scratch/fy2374-blah/toyota_one_shot/toyota_one_shot/ntu/ntu_no_border/one_shot/samples/' + image_path.split('/')[-1].split('_')[0] + '/' + (image_path.split('/')[-1]).replace('npy', 'png')
        else:
            image_path = '/hkfs/work/workspace/scratch/fy2374-blah/toyota_one_shot/toyota_one_shot/ntu/ntu_no_border/one_shot/test/'+ image_path.split('/')[-1].split('_')[0] + '/' + (image_path.split('/')[-1]).replace('npy', 'png')
        ...
        #data_numpy = torch.cat([data_numpy, data_numpy[:1]], dim=0)
        ...
        #data_numpy = torch.Tensor(np.resize(data_numpy,(256,256,2)).astype(np.float64)).permute(2,0,1) #self.transforms(data_numpy)
        #bone_data_numpy = torch.Tensor(np.resize(bone_data_numpy,(256,256,2)).astype(np.float64)).permute(2,0,1)#self.transforms(bone_data_numpy)
        #vel_data_numpy = torch.Tensor(np.resize(vel_data_numpy,(256,256,2)).astype(np.float64)).permute(2,0,1) #self.transforms(vel_data_numpy)    
        return torch.stack([data_numpy, bone_data_numpy, vel_data_numpy], dim=0), torch.Tensor([label]).long()
