...
class Feeder(Dataset):
    def __init__(self, data_path=None, label_path=None, p_interval=1, mode='train', random_choose=False, random_shift=False,
        ...
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/data_info.pkl', 'rb')
        self.data_info = pkl.load(f)
        f.close()                  
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/key_info.pkl', 'rb')
        self.key_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/num_info.pkl', 'rb')
        ...
        val_info = pkl.load(f)
        f.close()
        f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/length_info.pkl', 'rb')
        ...
        if mode == 'train':
            f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/oridata/train_ind.pkl', 'rb')
            self.index_list = pkl.load(f)
            f.close() 
            #self.index_list = [self.key_info.index(item) for item in train_info] 
        elif mode == 'val':
            f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/oridata/val_ind.pkl', 'rb')
            self.index_list = pkl.load(f)
            f.close() 
            #self.index_list = [self.key_info.index(item) for item in val_info]
        else:
            f = open('/hkfs/work/workspace/scratch/fy2374-blah/occ_3d/oridata/oridata/test_ind.pkl', 'rb')
            self.index_list = pkl.load(f)
            f.close() 
    ...
    def __getitem__(self, ind):
        ...
        #data_numpy = self.gaussian_noise(data_numpy)
        #bone_data_numpy = self.gaussian_noise(bone_data_numpy)
        #vel_data_numpy = self.gaussian_noise(vel_data_numpy)
        #data_numpy = torch.Tensor(np.resize(data_numpy,(256,256,3)).astype(np.float64)).permute(2,0,1) #self.transforms(data_numpy)
        #bone_data_numpy = torch.Tensor(np.resize(bone_data_numpy,(256,256,3))).permute(2,0,1)#self.transforms(bone_data_numpy)
        #vel_data_numpy = torch.Tensor(np.resize(vel_data_numpy,(256,256,3))).permute(2,0,1) #self.transforms(vel_data_numpy)    
        return  torch.stack([data_numpy, bone_data_numpy, vel_data_numpy], dim=1).permute(0,1,2,3), torch.Tensor([label]).long()
