...
class HookContainer: 
    ...
    def save_models_and_eval(self, trainer, dataset_dict, model_folder, test_interval, tester, collate_fn):
        epoch = trainer.epoch
        tester.test(dataset_dict, epoch, trainer.models["trunk"], embedder_model = trainer.models["embedder"], classifier_model = trainer.models["classifier"], plits_to_eval=list(dataset_dict.keys()), collate_fn=collate_fn)
